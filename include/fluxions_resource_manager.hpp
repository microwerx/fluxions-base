...
#define TRESOURCEMANAGER_VECTOR_IMPLEMENTATION 1
...
namespace Fluxions {
	...
	private:
#ifdef TRESOURCEMANAGER_VECTOR_IMPLEMENTATION
		std::vector<T*> ptrs_;
#endif
	...
	public:
		...
		std::vector<T*>& ptrs() { return ptrs_; }
		const std::vector<T*>& ptrs() const { return ptrs_; }
	};
...
#ifdef TRESOURCEMANAGER_VECTOR_IMPLEMENTATION
		ptrs_.clear();
		ptrs_.push_back(&defaultObject_);
#endif
...
#ifdef TRESOURCEMANAGER_VECTOR_IMPLEMENTATION
			ptrs_.push_back(&resources_[largestHandle_]);
#endif
...
#ifdef TRESOURCEMANAGER_VECTOR_IMPLEMENTATION
		if (handle < largestHandle_ && ptrs_[handle]) {
			return *ptrs_[handle];
		}
#endif
